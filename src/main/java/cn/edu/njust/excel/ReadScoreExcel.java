...
public class ReadScoreExcel {
	...
	public List<Score> readXls(String path, String session) throws IOException {
        ...
        for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
            ...
            // 循环行RowhssfSheet.getLastRowNum()
            for (int rowNum = 3; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
            	
                HSSFRow hssfRow = hssfSheet.getRow(rowNum);
                if (hssfRow != null && getValue(hssfRow.getCell(1)) != "") {
                    score = new Score();
                    ...
                    try {
						...
						//return null;session
						  ...
						  if(!stuID.equals("NO")) { 
						  score.setSession(session); score.setStuID(stuID);
						  score.setTotal_score((int) Double.parseDouble(getValue(all_sc)));
						  score.setScore_1((int) Double.parseDouble(getValue(first)));
						  score.setScore_2((int) Double.parseDouble(getValue(second)));
						  score.setScore_3((int) Double.parseDouble(getValue(third)));
						  score.setScore_4((int) Double.parseDouble(getValue(fourth)));
						  score.setScore_5((int) Double.parseDouble(getValue(fifth)));
						  //
						  score.setCur_rank(getValue(rank_1)); score.setAll_rank(getValue(rank_all));
						  list.add(score); }
